...
public class ApartmentController {
    ...
    public Result saveOrUpdate(@RequestBody ApartmentSubmitVo apartmentSubmitVo) {
        service.saveOrUpdateApartment(apartmentSubmitVo);
                                                                       //因为ApartmentInfoService类继承了IService<ApartmentInfo>这个接口，所以可以能够直接操作ApartmentInfo实体类对应的数据库表，而无需手动实现这些方法。
        return Result.ok();
    }
    ...
    public Result<IPage<ApartmentItemVo>> pageItem(@RequestParam long current, @RequestParam long size, ApartmentQueryVo queryVo) {
        ...
        IPage<ApartmentItemVo> result = service.pageItem(page, queryVo); //IPage<T> 是 MyBatis-Plus 中用于分页查询的一个接口，它用于封装分页的结果数据。
        return Result.ok(result);
    }

    @Operation(summary = "根据ID获取公寓详细信息")//用于修改公寓的数据回显上
